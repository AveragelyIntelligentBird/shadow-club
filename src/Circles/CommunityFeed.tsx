...
export default function CommunityFeed() {
    ...
    const fetchProfilesInCircle = async () => {
        const profiles = await profileClient.findProfilesInCircle(id || "");
        setMembers(profiles);
    }
    ...
    return (
        <div id="community-feed" className='wd-bg-ebony'>
            ...
            <div className="d-lg-flex">
                        ...
                        : (user && user?.memberOf.includes(id || '')) ? <Feed posts={posts}/>
                            ...
                            : (user) ? <div className='private-message'>
                                    <h2 className='private-message-title'>
                                        ...
                                        {moderators[0] && (
                                            <>
                                                &#160;Contact <Link className='private-message-title'
                                                                    to={`/Profile/${moderators[0]._id}`}>{moderators[0].username}</Link>to
                                                join.
                                            </>)}
                                    </h2>
                                </div>
                ...
                <MemberList cid={id || "0"} members={members} setMembers={setMembers}/>
            </div>
        </div>
    );
}